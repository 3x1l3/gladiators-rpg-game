...
XMLWrapper::XMLWrapper(const std::string& filename) throw() {
  result = doc.load_file(filename.c_str());
}

std::string XMLWrapper::getResult()  throw(){
  std::string val = result.description();
  return val;
}

bool XMLWrapper::isValid(std::string type) throw() {

    std::string first_child = doc.first_child().name();
    std::string attribute = doc.first_child().first_attribute().value();
    if(first_child != "GLAD")
        throw invalid_file("Your filename was incorrect, or your file is corrupt.");

  return (first_child == "GLAD" && attribute==type?true:false);
}

void XMLWrapper::LoadSaveFile() throw() {
...
std::vector<int> XMLWrapper::explodetoInts (const std::string original, const std::string exploder=".") throw() {
	std::string tmp;
	tmp=original;
	int num, loc;
	num=1;
...
std::vector<std::vector<int> > XMLWrapper::getItemVector() throw() {
  return items;
}

std::vector<std::vector<std::string> > XMLWrapper::getCharacterVector() throw() {
  return characters;
}

int XMLWrapper::getMoney() throw() {
  return money;
}

int XMLWrapper::getBattleNumber() throw() {
  return battlenumber;
}
